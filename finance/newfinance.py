...
complexities_btc = []
for month, group in df_btc.groupby(pd.Grouper(key='Date', freq='M')):
    # Calculate the mean and standard deviation of the price difference
    # mean = group['Open'].mean() - group['Close'].mean()
    # std = group['Open'].std() - group['Close'].std()
    # # Define the threshold based on the number of standard deviations
    # threshold = mean + 2 * std
    # # Create a new binary sequence array based on the threshold value and the price difference
    # binary_sequence = np.where(group['Open'] - group['Close'] > threshold, 1, 0)
